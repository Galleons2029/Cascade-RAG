...
Set required API keys in your `.env` (root-level config is reused):
...
In LangGraph Studio, choose graph `deep_research_agent` (registered in root `langgraph.json`).
